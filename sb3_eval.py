...
def eval(model, num_episodes, env):
    ...
    for i in tqdm(range(num_episodes)):
        ...
        while not done:
            ...
            min_obstacle_dist = min(
                min_obstacle_dist, info["MINIMUM_OBSTACLE_DISTANCE"]
            )
            ...
            if info["TIME_TO_COLLISION"] != -1:
                ...
                avg_minimum_time_to_collision += env.EPISODE_LENGTH
            episode_reward += reward
    ...
    print(
        f"Average average_obstacle distance: {total_avg_obstacle_distance/num_episodes}"
    )
    print(f"Average psc: {total_psc/num_episodes}")
    print(f"Average closest_human_dist: {closest_human_dist/num_episodes}")
    print(
        f"Average minimum_time_to_collision: {total_minimum_time_to_collision/num_episodes}"
    )
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "-n", "--num_episodes", type=int, required=True, help="number of episodes"
    )
    ap.add_argument(
        "-w", "--weight_path", type=str, required=True, help="path to weight file"
    )
    ap.add_argument(
