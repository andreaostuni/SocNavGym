...
import socnavgym
from socnavgym.wrappers import (
...
def test_env():
    ...
    for i in range(10):
        env = gym.make(
            ...
            + "/../environment_configs/test_env.yaml",
        )
        env_world = WorldFrameObservations(env)
        env_noise = NoisyObservations(
            env, np.random.random(), np.random.random() + 1e-5
        )
        env_partial = PartialObservations(
            env, np.random.random() * np.pi * 2, np.random.randint(1, 5)
        )
