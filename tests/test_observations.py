...
def test_observations():
    ...
    answer = [
        ...
        [4, 4, 0, 1],
        [-2, 0, 0.5, np.sqrt(3) / 2],
        [-4 * np.sqrt(2), 0, -1 / np.sqrt(2), 1 / np.sqrt(2)],
    ]

    env = gym.make(
        "SocNavGym-v1",
        config=os.path.dirname(os.path.abspath(__file__))
        + "/../environment_configs/test2.yaml",
    )
    ...
    for i in range(len(robot)):
        ...
        obs = obs["laptops"]
        assert (
            (np.abs(obs[6] - answer[i][0]) <= 1e-6)
            and (np.abs(obs[7] - answer[i][1]) <= 1e-6)
            and (np.abs(obs[8] - answer[i][2]) <= 1e-6)
            and (np.abs(obs[9] - answer[i][3]) <= 1e-6)
        )
