...
from socnavgym.envs.utils.utils import (
...
MAX_ORIENTATION_CHANGE = math.pi / 2.0
...
class SocNavGymObject(Enum):
    ...
    HUMAN_HUMAN_INTERACTION_DYNAMIC_NON_DISPERSING = (
        "HUMAN_HUMAN_INTERACTION_DYNAMIC_NON_DISPERSING"
    )
    HUMAN_HUMAN_INTERACTION_STATIC = "HUMAN_HUMAN_INTERACTION_STATIC"
    HUMAN_HUMAN_INTERACTION_STATIC_NON_DISPERSING = (
        "HUMAN_HUMAN_INTERACTION_STATIC_NON_DISPERSING"
    )
    ...
    def get_class(self):
        if self == SocNavGymObject.ROBOT:
            return Robot
        if self == SocNavGymObject.DYNAMIC_HUMAN:
            return Human
        if self == SocNavGymObject.STATIC_HUMAN:
            return Human
        if self == SocNavGymObject.PLANT:
            return Plant
        if self == SocNavGymObject.TABLE:
            return Table
        if self == SocNavGymObject.LAPTOP:
            return Laptop
        if self == SocNavGymObject.WALL:
            return Wall
        if self == SocNavGymObject.HUMAN_HUMAN_INTERACTION_DYNAMIC:
            return Human_Human_Interaction
        if self == SocNavGymObject.HUMAN_HUMAN_INTERACTION_DYNAMIC_NON_DISPERSING:
            return Human_Human_Interaction
        if self == SocNavGymObject.HUMAN_HUMAN_INTERACTION_STATIC:
            return Human_Human_Interaction
        if self == SocNavGymObject.HUMAN_HUMAN_INTERACTION_STATIC_NON_DISPERSING:
            return Human_Human_Interaction
        if self == SocNavGymObject.HUMAN_LAPTOP_INTERACTION:
            return Human_Laptop_Interaction
        if self == SocNavGymObject.HUMAN_LAPTOP_INTERACTION_NON_DISPERSING:
            return Human_Laptop_Interaction
...
class SocNavEnv_v1(gym.Env):
    ...
    def __init__(self, config: str = None) -> None:
        ...
        assert (
            config is not None
        ), 'Argument config_path is None. Please call gym.make("SocNavGym-v1", config_path=path_to_config)'
    ...
    def _configure(self, config_path):
        ...
        assert (
            self.ROBOT_RADIUS > 0 and self.GOAL_RADIUS > 0
        ...
        assert (
            self.HUMAN_POLICY == "random"
            or self.HUMAN_POLICY == "orca"
            or self.HUMAN_POLICY == "sfm"
        ), 'human_policy should be "random", or "orca" or "sfm"'
        ...
        self.LAPTOP_RADIUS = np.sqrt(
            (self.LAPTOP_LENGTH / 2) ** 2 + (self.LAPTOP_WIDTH / 2) ** 2
        )
        ...
        self.TABLE_RADIUS = np.sqrt(
            (self.TABLE_LENGTH / 2) ** 2 + (self.TABLE_WIDTH / 2) ** 2
        )
        ...
        self.INTERACTION_RADIUS = config["human-human-interaction"][
            "interaction_radius"
        ]
        self.INTERACTION_GOAL_RADIUS = config["human-human-interaction"][
            "interaction_goal_radius"
        ]
        self.INTERACTION_NOISE_VARIANCE = config["human-human-interaction"][
            "noise_variance"
        ]

        # human-laptop-interaction
        self.HUMAN_LAPTOP_DISTANCE = config["human-laptop-interaction"][
            "human_laptop_distance"
        ]
        ...
        assert (
            self.MIN_STATIC_HUMANS <= self.MAX_STATIC_HUMANS
        ), "min_static_humans should be less than or equal to max_static_humans"

        self.MIN_DYNAMIC_HUMANS = config["env"]["min_dynamic_humans"]
        self.MAX_DYNAMIC_HUMANS = config["env"]["max_dynamic_humans"]
        assert (
            self.MIN_DYNAMIC_HUMANS <= self.MAX_DYNAMIC_HUMANS
        ), "min_dynamic_humans should be less than or equal to max_dynamic_humans"
        ...
        assert (
            self.MIN_TABLES <= self.MAX_TABLES
        ), "min_tables should be less than or equal to max_tables"

        self.MIN_PLANTS = config["env"]["min_plants"]
        self.MAX_PLANTS = config["env"]["max_plants"]
        assert (
            self.MIN_PLANTS <= self.MAX_PLANTS
        ), "min_plants should be less than or equal to max_plants"

        self.MIN_LAPTOPS = config["env"]["min_laptops"]
        self.MAX_LAPTOPS = config["env"]["max_laptops"]
        assert (
            self.MIN_LAPTOPS <= self.MAX_LAPTOPS
        ), "min_laptops should be less than or equal to max_laptops"

        self.MIN_H_H_DYNAMIC_INTERACTIONS = config["env"][
            "min_h_h_dynamic_interactions"
        ]
        self.MAX_H_H_DYNAMIC_INTERACTIONS = config["env"][
            "max_h_h_dynamic_interactions"
        ]
        assert self.MIN_H_H_DYNAMIC_INTERACTIONS <= self.MAX_H_H_DYNAMIC_INTERACTIONS

        self.MIN_H_H_DYNAMIC_INTERACTIONS_NON_DISPERSING = config["env"][
            "min_h_h_dynamic_interactions_non_dispersing"
        ]
        self.MAX_H_H_DYNAMIC_INTERACTIONS_NON_DISPERSING = config["env"][
            "max_h_h_dynamic_interactions_non_dispersing"
        ]
        assert (
            self.MIN_H_H_DYNAMIC_INTERACTIONS_NON_DISPERSING
            <= self.MAX_H_H_DYNAMIC_INTERACTIONS_NON_DISPERSING
        )
        ...
        self.MIN_H_H_STATIC_INTERACTIONS_NON_DISPERSING = config["env"][
            "min_h_h_static_interactions_non_dispersing"
        ]
        self.MAX_H_H_STATIC_INTERACTIONS_NON_DISPERSING = config["env"][
            "max_h_h_static_interactions_non_dispersing"
        ]
        assert (
            self.MIN_H_H_STATIC_INTERACTIONS_NON_DISPERSING
            <= self.MAX_H_H_STATIC_INTERACTIONS_NON_DISPERSING
        )

        self.MIN_HUMAN_IN_H_H_INTERACTIONS = config["env"][
            "min_human_in_h_h_interactions"
        ]
        self.MAX_HUMAN_IN_H_H_INTERACTIONS = config["env"][
            "max_human_in_h_h_interactions"
        ]
        assert (
            self.MIN_HUMAN_IN_H_H_INTERACTIONS <= self.MAX_HUMAN_IN_H_H_INTERACTIONS
        ), "min_human_in_h_h_interactions should be less than or equal to max_human_in_h_h_interactions"

        self.MIN_H_L_INTERACTIONS = config["env"]["min_h_l_interactions"]
        self.MAX_H_L_INTERACTIONS = config["env"]["max_h_l_interactions"]
        assert (
            self.MIN_H_L_INTERACTIONS <= self.MAX_H_L_INTERACTIONS
        ), "min_h_l_interactions should be lesser than or equal to max_h_l_interactions"

        self.MIN_H_L_INTERACTIONS_NON_DISPERSING = config["env"][
            "min_h_l_interactions_non_dispersing"
        ]
        self.MAX_H_L_INTERACTIONS_NON_DISPERSING = config["env"][
            "max_h_l_interactions_non_dispersing"
        ]
        assert (
            self.MIN_H_L_INTERACTIONS_NON_DISPERSING
            <= self.MAX_H_L_INTERACTIONS_NON_DISPERSING
        ), "min_h_l_interactions_non_dispersing should be lesser than or equal to max_h_l_interactions_non_dispersing"

        self.get_padded_observations = config["env"]["get_padded_observations"]
        assert (
            self.get_padded_observations == True
            or self.get_padded_observations == False
        ), "get_padded_observations should be either True or False"

        self.set_shape = config["env"]["set_shape"]
        assert (
            self.set_shape == "random"
            or self.set_shape == "square"
            or self.set_shape == "rectangle"
            or self.set_shape == "L"
            or self.set_shape == "no-walls"
        ), 'set shape can be "random", "square", "rectangle", "L", or "no-walls"'
        ...
        assert (
            self.CROWD_DISPERSAL_PROBABILITY >= 0
            and self.CROWD_DISPERSAL_PROBABILITY <= 1.0
        ), "Probability should be within [0, 1]"

        self.HUMAN_LAPTOP_DISPERSAL_PROBABILITY = config["env"][
            "human_laptop_dispersal_probability"
        ]
        assert (
            self.HUMAN_LAPTOP_DISPERSAL_PROBABILITY >= 0
            and self.HUMAN_LAPTOP_DISPERSAL_PROBABILITY <= 1.0
        ), "Probability should be within [0, 1]"

        self.CROWD_FORMATION_PROBABILITY = config["env"]["crowd_formation_probability"]
        assert (
            self.CROWD_FORMATION_PROBABILITY >= 0
            and self.CROWD_FORMATION_PROBABILITY <= 1.0
        ), "Probability should be within [0, 1]"

        self.HUMAN_LAPTOP_FORMATION_PROBABILITY = config["env"][
            "human_laptop_formation_probability"
        ]
        assert (
            self.HUMAN_LAPTOP_FORMATION_PROBABILITY >= 0
            and self.HUMAN_LAPTOP_FORMATION_PROBABILITY <= 1.0
        ), "Probability should be within [0, 1]"
        ...
        reward_api_class = (
            SourceFileLoader(module, self.REWARD_PATH).load_module().RewardAPI
        )
        try:
            self.reward_class = reward_module.Reward
        except AttributeError:
            print(
                f"No class named Reward found in {self.REWARD_PATH}. Please name your reward function class as Reward!"
            )
            sys.exit(0)

        assert issubclass(
            self.reward_class, reward_api_class
        ), "Please make Reward class a subclass of RewardAPI class"
    ...
    def randomize_params(self):
        ...
        self.NUMBER_OF_STATIC_HUMANS = random.randint(
            self.MIN_STATIC_HUMANS, self.MAX_STATIC_HUMANS
        )  # number of static humans in the env
        self.NUMBER_OF_DYNAMIC_HUMANS = random.randint(
            self.MIN_DYNAMIC_HUMANS, self.MAX_DYNAMIC_HUMANS
        )  # number of static humans in the env
        self.NUMBER_OF_PLANTS = random.randint(
            self.MIN_PLANTS, self.MAX_PLANTS
        )  # number of plants in the env
        self.NUMBER_OF_TABLES = random.randint(
            self.MIN_TABLES, self.MAX_TABLES
        )  # number of tables in the env
        self.NUMBER_OF_LAPTOPS = random.randint(
            self.MIN_LAPTOPS, self.MAX_LAPTOPS
        )  # number of laptops in the env. Laptops will be sampled on tables
        self.NUMBER_OF_H_H_DYNAMIC_INTERACTIONS = random.randint(
            self.MIN_H_H_DYNAMIC_INTERACTIONS, self.MAX_H_H_DYNAMIC_INTERACTIONS
        )  # number of dynamic human-human interactions
        self.NUMBER_OF_H_H_DYNAMIC_INTERACTIONS_NON_DISPERSING = random.randint(
            self.MIN_H_H_DYNAMIC_INTERACTIONS_NON_DISPERSING,
            self.MAX_H_H_DYNAMIC_INTERACTIONS_NON_DISPERSING,
        )  # number of dynamic human-human interactions that do not disperse
        self.NUMBER_OF_H_H_STATIC_INTERACTIONS = random.randint(
            self.MIN_H_H_STATIC_INTERACTIONS, self.MAX_H_H_STATIC_INTERACTIONS
        )  # number of static human-human interactions
        self.NUMBER_OF_H_H_STATIC_INTERACTIONS_NON_DISPERSING = random.randint(
            self.MIN_H_H_STATIC_INTERACTIONS_NON_DISPERSING,
            self.MAX_H_H_STATIC_INTERACTIONS_NON_DISPERSING,
        )  # number of static human-human interactions that do not disperse
        ...
        for _ in range(self.NUMBER_OF_H_H_DYNAMIC_INTERACTIONS):
            self.humans_in_h_h_dynamic_interactions.append(
                random.randint(
                    self.MIN_HUMAN_IN_H_H_INTERACTIONS,
                    self.MAX_HUMAN_IN_H_H_INTERACTIONS,
                )
            )
        ...
        self.NUMBER_OF_H_L_INTERACTIONS = random.randint(
            self.MIN_H_L_INTERACTIONS, self.MAX_H_L_INTERACTIONS
        )  # number of human laptop interactions
        self.NUMBER_OF_H_L_INTERACTIONS_NON_DISPERSING = random.randint(
            self.MIN_H_L_INTERACTIONS_NON_DISPERSING,
            self.MAX_H_L_INTERACTIONS_NON_DISPERSING,
        )  # number of human laptop interactions that do not disperse
        self.TOTAL_H_L_INTERACTIONS = (
            self.NUMBER_OF_H_L_INTERACTIONS
            + self.NUMBER_OF_H_L_INTERACTIONS_NON_DISPERSING
        )

        # total humans
        self.total_humans = self.NUMBER_OF_STATIC_HUMANS + self.NUMBER_OF_DYNAMIC_HUMANS
        for i in self.humans_in_h_h_dynamic_interactions:
            self.total_humans += i
        for i in self.humans_in_h_h_static_interactions:
            self.total_humans += i
        for i in self.humans_in_h_h_dynamic_interactions_non_dispersing:
            self.total_humans += i
        for i in self.humans_in_h_h_static_interactions_non_dispersing:
            self.total_humans += i
    ...
    def MAX_OBSERVATION_LENGTH(self):
        return (
            self.MAX_HUMANS + self.MAX_LAPTOPS + self.MAX_PLANTS + self.MAX_TABLES
            ...
            "robot": spaces.Box(
                low=np.array(
                    [
                        ...
                        0,
                        0,
                        -self.MAP_X * np.sqrt(2),
                        -self.MAP_Y * np.sqrt(2),
                        -self.ROBOT_RADIUS,
                    ],
                    dtype=np.float32,
                ),
            ...
            d["humans"] = spaces.Box(
                low=np.array(
                    [
                        ...
                        -(self.MAX_ADVANCE_HUMAN + self.MAX_ADVANCE_ROBOT) * np.sqrt(2),
                        -2 * np.pi / self.TIMESTEP,
                        0,
                    ]
                    * (
                        (
                            self.MAX_HUMANS
                            + (
                                self.MAX_H_L_INTERACTIONS
                                + self.MAX_H_L_INTERACTIONS_NON_DISPERSING
                            )
                            + (
                                (
                                    self.MAX_H_H_DYNAMIC_INTERACTIONS
                                    + self.MAX_H_H_DYNAMIC_INTERACTIONS_NON_DISPERSING
                                )
                                * self.MAX_HUMAN_IN_H_H_INTERACTIONS
                            )
                            + (
                                (
                                    self.MAX_H_H_STATIC_INTERACTIONS
                                    + self.MAX_H_H_STATIC_INTERACTIONS_NON_DISPERSING
                                )
                                * self.MAX_HUMAN_IN_H_H_INTERACTIONS
                            )
                ...
                shape=(
                    (
                        (self.robot.one_hot_encoding.shape[0] + 8)
                        * (
                            (
                                self.MAX_HUMANS
                                + (
            ...
            d["laptops"] = spaces.Box(
                low=np.array(
                    [
                        ...
                        -1.0,
                        -self.LAPTOP_RADIUS,
                        -(self.MAX_ADVANCE_ROBOT) * np.sqrt(2),
                        -self.MAX_ROTATION,
                        0,
                    ]
                    * (
                        ...
                        if self.get_padded_observations
                        else (self.NUMBER_OF_LAPTOPS + self.TOTAL_H_L_INTERACTIONS)
                    ),
                    dtype=np.float32,
                ),
                high=np.array(
    ...
    def transformation_matrix(self):
        ...
        assert (
            self.robot.x is not None
            and self.robot.y is not None
            and self.robot.orientation is not None
    ...
    def is_entity_present(self) -> Dict[str, bool]:
        ...
        if (not self.get_padded_observations) and (self.total_humans == 0):
            d["humans"] = False
        if (not self.get_padded_observations) and (self.NUMBER_OF_PLANTS == 0):
            d["plants"] = False
        if (not self.get_padded_observations) and (self.NUMBER_OF_TABLES == 0):
            d["tables"] = False
        if (not self.get_padded_observations) and (
            (self.NUMBER_OF_LAPTOPS + self.TOTAL_H_L_INTERACTIONS) == 0
        ):
            d["laptops"] = False
        if self.shape == "no-walls":
            d["walls"] = False
    ...
    def human_transformation_matrix(self, human: Human):
        ...
        assert (
            human.x is not None
            and human.y is not None
            and human.orientation is not None
        ), "Human coordinates or orientation are None type"
        # initalizing the matrix
        tm = np.zeros((3, 3), dtype=np.float32)
        # filling values as described
        tm[2, 2] = 1
    ...
    def get_robot_frame_coordinates(self, coord):
        ...
        coord_in_robot_frame = (
            self.transformation_matrix @ homogeneous_coordinates.T
        ).T
    ...
    def is_entity_visible_in_human_frame(self, human: Human, entity: Object):
        ...
        for steps in range(1001):
            ...
            sector_points.append(
                [human.x + radius * np.cos(angle), human.y + radius * np.sin(angle)]
            )
        ...
        if entity.name == "plant" or entity.name == "robot":
            assert (
                entity.x != None and entity.y != None and entity.radius != None
            ), "Attributes are None type"
            other_obj = Point((entity.x, entity.y)).buffer(entity.radius)

        elif entity.name == "human":
            assert (
                entity.x != None and entity.y != None and entity.width != None
            ), "Attributes are None type"
            ...
            assert (
                entity.x != None
                and entity.y != None
                and entity.width != None
                and entity.length != None
                and entity.orientation != None
            ), "Attributes are None type"
            other_obj = Polygon(
                get_coordinates_of_rotated_rectangle(
                    entity.x, entity.y, entity.orientation, entity.length, entity.width
                )
            )
    ...
    def _get_entity_obs(self, object):
        ...
        assert (
            (object.x is not None)
            and (object.y is not None)
            and (object.orientation is not None)
        ), f"{object.name}'s coordinates or orientation are None type"
        ...
        def _get_wall_obs(wall: Wall, size: float):
            ...
            if wall.length % size != 0:
                ...
                centers.append(
                    (
                        right_x - np.cos(wall.orientation) * length / 2,
                        right_y - np.sin(wall.orientation) * length / 2,
                    )
                )
            ...
            for center, length in zip(centers, lengths):
                ...
                obs = np.concatenate(
                    (
                        obs,
                        self.get_robot_frame_coordinates(
                ...
                relative_speeds = np.array(
                    [
                        -np.sqrt(self.robot.vel_x**2 + self.robot.vel_y**2),
                        -self.robot.vel_a,
                    ],
                    dtype=np.float32,
                )
            ...
            wall_coordinates = self.get_robot_frame_coordinates(
                np.array([[wall.x, wall.y]])
            ).flatten()
        ...
        output = np.concatenate((output, object.one_hot_encoding), dtype=np.float32)
        ...
        output = np.concatenate(
            (
                ...
                np.array(
                    [
                        (np.sin(object.orientation - self.robot.orientation)),
                        np.cos(object.orientation - self.robot.orientation),
                    ]
                ),
            ),
            dtype=np.float32,
        ...
        output = np.concatenate(
            (output, np.array([radius], dtype=np.float32)), dtype=np.float32
        )

        robot_vel_x = self.robot.vel_x * np.cos(
            self.robot.orientation
        ) + self.robot.vel_y * np.cos(self.robot.orientation + np.pi / 2)
        robot_vel_y = self.robot.vel_x * np.sin(
            self.robot.orientation
        ) + self.robot.vel_y * np.sin(self.robot.orientation + np.pi / 2)

        # relative speeds for static objects
        relative_speeds = np.array(
            [-np.sqrt(self.robot.vel_x**2 + self.robot.vel_y**2), -self.robot.vel_a],
            dtype=np.float32,
        )
        ...
        if object.name == "human":  # the only dynamic object
            ...
            relative_speeds[0] = np.sqrt(
                (object.speed * np.cos(object.orientation) - robot_vel_x) ** 2
                + (object.speed * np.sin(object.orientation) - robot_vel_y) ** 2
            )
            relative_speeds[1] = (
                np.arctan2(
                    np.sin(object.orientation - self.robot.orientation),
                ...
                - self._prev_observations[object.id].theta
            ) / self.TIMESTEP
        ...
        if object.name == "human":
            robot_in_human_frame = self.get_human_frame_coordinates(
                object, np.array([[self.robot.x, self.robot.y]])
            ).flatten()
            robot_x = robot_in_human_frame[0]
            robot_y = robot_in_human_frame[1]

            if (
                np.arctan2(robot_y, robot_x) >= -self.HUMAN_GAZE_ANGLE / 2
                and np.arctan2(robot_y, robot_x) <= self.HUMAN_GAZE_ANGLE / 2
            ):
                gaze = 1.0
        ...
        assert (
            self.entity_obs_dim == output.flatten().shape[-1]
        ), "The value of self.entity_obs_dim needs to be changed"
    ...
    def _get_obs(self):
        ...
        goal_in_robot_frame = self.get_robot_frame_coordinates(
            np.array([[self.robot.goal_x, self.robot.goal_y]], dtype=np.float32)
        )
        ...
        robot_obs = np.concatenate(
            (self.robot.one_hot_encoding, robot_obs), dtype=np.float32
        )

        # adding the radius of the robot to the robot's observation
        robot_obs = np.concatenate(
            (robot_obs, np.array([self.ROBOT_RADIUS], dtype=np.float32))
        ).flatten()
        ...
        for i in (
            self.moving_interactions + self.static_interactions + self.h_l_interactions
        ):
        ...
        if self.get_padded_observations:
            ...
            human_obs = np.concatenate(
                (
                    human_obs,
                    np.zeros(
                        self.observation_space["humans"].shape[0] - human_obs.shape[0]
                    ),
                ),
                dtype=np.float32,
            )
        ...
        if self.get_padded_observations:
            # padding with zeros
            laptop_obs = np.concatenate(
                (
                    laptop_obs,
                    np.zeros(
                        self.observation_space["laptops"].shape[0] - laptop_obs.shape[0]
                    ),
                ),
                dtype=np.float32,
            )
        ...
        if self.get_padded_observations:
            # padding with zeros
            table_obs = np.concatenate(
                (
                    table_obs,
                    np.zeros(
                        self.observation_space["tables"].shape[0] - table_obs.shape[0]
                    ),
                ),
                dtype=np.float32,
            )
        ...
        if self.get_padded_observations:
            # padding with zeros
            plant_obs = np.concatenate(
                (
                    plant_obs,
                    np.zeros(
                        self.observation_space["plants"].shape[0] - plant_obs.shape[0]
                    ),
                ),
                dtype=np.float32,
            )
    ...
    def get_desired_force(self, human: Human):
        e_d = np.array(
            [(human.goal_x - human.x), (human.goal_y - human.y)], dtype=np.float32
        )
    ...
    def get_obstacle_force(self, human: Human, obstacle: Object, r0):
        ...
        if obstacle.name == "plant" or obstacle.name == "robot":
            distance = (
                np.sqrt((obstacle.x - human.x) ** 2 + (obstacle.y - human.y) ** 2)
                - obstacle.radius
                - human.width / 2
            )
            ...
            px, py = get_nearest_point_from_rectangle(
                obstacle.x,
                obstacle.y,
                obstacle.length,
                obstacle.width,
            ...
            if np.linalg.norm(e_o) != 0:
                e_o /= np.linalg.norm(e_o)
            distance = (
                np.sqrt((human.x - px) ** 2 + (human.y - py) ** 2) - human.width / 2
            )
    ...
    def get_interaction_force(
        self, human1: Human, human2: Human, gamma, n, n_prime, lambd
    ):
        ...
        v_ij = np.array(
            [
                (human2.speed * np.cos(human2.orientation))
                - (human1.speed * np.cos(human1.orientation)),
                (human2.speed * np.sin(human2.orientation))
                - (human1.speed * np.sin(human1.orientation)),
            ]
        )
        ...
        f_ij = -np.exp(-d_ij / B) * (
            np.exp(-((n_prime * B * theta_ij) ** 2)) * t_ij
            + np.exp(-((n * B * theta_ij) ** 2)) * n_ij
        )
        return f_ij

    def compute_sfm_velocity(
        self, human: Human, w1=1 / np.sqrt(3), w2=1 / np.sqrt(3), w3=1 / np.sqrt(3)
    ):
        ...
        for h in self.static_humans + self.dynamic_humans:
            ...
            elif self.is_entity_visible_in_human_frame(human, h):
                visible_humans.append(h)

        for plant in self.plants:
            # if visible append to visible list
            if self.is_entity_visible_in_human_frame(human, plant):
                visible_plants.append(plant)

        for table in self.tables:
            # if visible append to visible list
            if self.is_entity_visible_in_human_frame(human, table):
                visible_tables.append(table)

        for laptop in self.laptops:
            # if visible append to visible list
            if self.is_entity_visible_in_human_frame(human, laptop):
                visible_laptops.append(laptop)

        for wall in self.walls:
            # if visible append to visible list
            if self.is_entity_visible_in_human_frame(human, wall):
                visible_walls.append(wall)

        for interaction in self.moving_interactions:
            # if visible append to visible list
            if self.is_entity_visible_in_human_frame(human, interaction):
        ...
        for other_human in visible_humans:
            if other_human.id == human.id:
                continue
            else:
                f += w3 * self.get_interaction_force(
                    human,
                    other_human,
                    self.sfm_gamma,
                    self.sfm_n,
    ...
    def compute_orca_velocity(self, human: Human):
        ...
        if np.random.random() <= human.prob_to_avoid_robot:
            ...
            # preferred velocity is towards the goal
            pref_vel = np.array(
                [self.robot.goal_x - self.robot.x, self.robot.goal_y - self.robot.y],
                dtype=np.float32,
            )
            # normalising the velocity
            if not np.linalg.norm(pref_vel) == 0:
        ...
        for i in visible_moving_interactions:
            ...
            sim.setAgentNeighborDist(
                h, 2 * (self.INTERACTION_RADIUS + self.HUMAN_DIAMETER)
            )
    ...
    def compute_orca_velocity_robot(self, robot: Robot):
        ...
        # preferred velocity is towards the goal
        pref_vel = np.array(
            [robot.goal_x - robot.x, robot.goal_y - robot.y], dtype=np.float32
        )
        # normalising the velocity
        if not np.linalg.norm(pref_vel) == 0:
            pref_vel /= np.linalg.norm(pref_vel)
    ...
    def get_obstacle_corners(self, obs: Object):
        if obs.name == "laptop" or obs.name == "table":
            return get_coordinates_of_rotated_rectangle(
                obs.x, obs.y, obs.orientation, obs.length, obs.width
            )

        elif obs.name == "wall":
            return get_coordinates_of_rotated_rectangle(
                obs.x, obs.y, obs.orientation, obs.length, obs.thickness
            )
            ...
            return get_square_around_circle(
                obs.human.x, obs.human.y, self.HUMAN_DIAMETER / 2
            )
    ...
    def get_collision_force(self, entity_a, entity_b):
        ...
        delta_pos = np.array(
            [entity_a.x - entity_b.x, entity_a.y - entity_b.y], dtype=np.float32
        )
        ...
        if entity_a.name == "wall":
            if entity_a.orientation == np.pi / 2 or entity_a.orientation == -np.pi / 2:
                ...
                center_x = (
                    ...
                    + (
                        ...
                        else (-entity_a.thickness)
                    )
                )
                center_y = entity_b.y
                delta_pos = np.array(
                    [center_x - entity_b.x, center_y - entity_b.y], dtype=np.float32
                )

            elif (
                entity_a.orientation == 0
                or entity_a.orientation == np.pi
                or entity_a.orientation == -1 * np.pi
            ):
            ...
            if entity_b.orientation == np.pi / 2 or entity_b.orientation == -np.pi / 2:
                center_x = (
                    ...
                    + (
                        (entity_b.thickness)
                        if entity_a.x >= entity_b.x
                        else (-entity_b.thickness)
                    )
                )
                center_y = entity_a.y
                delta_pos = np.array(
                    [entity_a.x - center_x, entity_a.y - center_y], dtype=np.float32
                )

            elif (
                entity_b.orientation == 0
                or entity_b.orientation == np.pi
                or entity_b.orientation == -1 * np.pi
            ):
                center_x = entity_a.x
                center_y = (
                    2 * entity_b.y
        ...
        force_a = (
            +force if not entity_a.is_static else None
        )  # forces are applied only to dynamic objects
        force_b = (
            -force if not entity_b.is_static else None
        )  # forces are applied only to dynamic objects
        ...
        for human in all_humans:
            ...
            for object in self.plants + self.walls + self.tables + self.laptops:
                if human.collides(object):
                    ...
                    if human.set_new_orientation_with_limits(
                        orientation, MAX_ORIENTATION_CHANGE, self.TIMESTEP
                    ):
                        human.speed = min(
                            np.linalg.norm(entity_vel), self.MAX_ADVANCE_HUMAN
                        )
                    else:
                        human.speed = 0
    ...
    def step(self, action_pre):
        ...
        def process_action(act):
            ...
            action[0] = (
                (action[0] + 0.0) / 1.0
            ) * self.MAX_ADVANCE_ROBOT  # [-1, +1] --> [-MAX_ADVANCE, +MAX_ADVANCE]
            if action[1] != 0.0 and self.robot.type == "diff-drive":
            ...
            ):  # Advance must be less or equal self.MAX_ADVANCE_ROBOT
                action[0] = self.MAX_ADVANCE_ROBOT
            if (
                action[0] < -self.MAX_ADVANCE_ROBOT
            ):  # Advance must be less or equal self.MAX_ADVANCE_ROBOT
            ...
            ):  # Rotation must be higher than -self.MAX_ROTATION
                action[2] = -self.MAX_ROTATION
            elif (
                action[2] > +self.MAX_ROTATION
            ):  # Rotation must be lower than +self.MAX_ROTATION
                action[2] = +self.MAX_ROTATION
            return action
        ...
        if (not self.has_orca_robot_collided) and (
            not self.has_orca_robot_reached_goal
        ):
            vel = self.compute_orca_velocity_robot(self.robot_orca)
            if self.robot_orca.type == "holonomic":
                vel_x = vel[0] * np.cos(self.robot_orca.orientation) + vel[1] * np.sin(
                    self.robot_orca.orientation
                )
                vel_y = -vel[0] * np.sin(self.robot_orca.orientation) + vel[1] * np.cos(
                    self.robot_orca.orientation
                )
                vel_a = (
            ...
            self.robot_orca.vel_y = np.clip(
                vel_y, -self.MAX_ADVANCE_ROBOT, self.MAX_ADVANCE_ROBOT
            )
            self.robot_orca.vel_a = np.clip(
                vel_a, -self.MAX_ROTATION, self.MAX_ROTATION
            )
        ...
        for index, human in enumerate(self.dynamic_humans):
            ...
            if human.set_new_orientation_with_limits(
                ...
                if human.speed < self.SPEED_THRESHOLD and not (
                    self.crowd_forming and human.id in self.humans_forming_crowd.keys()
                ):
                    human.speed = 0
            else:
                human.speed = 0
        ...
        for human in self.dynamic_humans:
            if self.crowd_forming and human.id in self.humans_forming_crowd.keys():
                continue  # handling the humans forming a crowd separately
            if self.h_l_forming and human.id == self.h_l_forming_human.id:
                continue  # handling the human forming the human-laptop-interaction separately
            HALF_SIZE_X = self.MAP_X / 2.0 - self.MARGIN
            HALF_SIZE_Y = self.MAP_Y / 2.0 - self.MARGIN
        ...
        for i in self.moving_interactions:
            if i.has_reached_goal():
                ...
                HALF_SIZE_Y = self.MAP_Y / 2.0 - self.MARGIN
                o = self.sample_goal(
                    self.INTERACTION_GOAL_RADIUS, HALF_SIZE_X, HALF_SIZE_Y
                )
                if o is not None:
                    i.set_goal(o.x, o.y)
                    for human in i.humans:
        ...
        if (
            self.crowd_forming
        ):  # enter only when the environment is undergoing a crowd formation
            ...
            for human in self.humans_forming_crowd.values():
                if human.has_reached_goal(offset=0):
                    ...
                    human.orientation = self.upcoming_interaction.humans[
                        self.id_to_index[human.id]
                    ].orientation
                else:
        ...
        if (
            np.random.random() <= self.HUMAN_LAPTOP_DISPERSAL_PROBABILITY
            and len(self.dispersable_h_l_interaction_indices) > 0
        ):
        ...
        if (
            np.random.random() <= self.CROWD_FORMATION_PROBABILITY
            and not self.crowd_forming
            and not self.h_l_forming
        ):
            self.form_human_crowd()  # form a new human crowd

        if (
            np.random.random() <= self.HUMAN_LAPTOP_FORMATION_PROBABILITY
            and not self.crowd_forming
            and not self.h_l_forming
        ):
    ...
    def sample_goal(self, goal_radius, HALF_SIZE_X, HALF_SIZE_Y):
        ...
        while True:
            ...
            for obj in all_objects + list(
                self.goals.values()
            ):  # check if spawned object collides with any of the exisiting objects. It will not be rendered as a plant.
                if obj is None:
                    continue
        ...
        if static_human_count <= 1:
            ...
            for human in interaction.humans:
                if human.type == "static":
                    ...
                    self.goals[human.id] = Plant(
                        id=None, x=human.x, y=human.y, radius=self.HUMAN_GOAL_RADIUS
                    )
            ...
            for human in humans_to_remove:
                interaction.humans.remove(human)
    ...
    def disperse_moving_crowd(self, index):
        ...
        if len(static_humans) <= 1:
            for human in static_humans:
                ...
                human.set_goal(human.x, human.y)
                human.speed = 0
                self.static_humans.append(
                    human
                )  # note this is the static humans list of the env, and not the local list created above
    ...
    def disperse_human_laptop(self, index):
        ...
        self.dynamic_humans.append(
            interaction.human
        )  # does not make sense to keep the human static in front of the laptop, so pushing the human in dynamic humans list
    ...
    def form_human_crowd(self):
        ...
        if numHumans <= 1:  # cannot form a crowd with one or less humans
            ...
            if (
                len(self.dynamic_humans) < numHumans
            ):  # check if the number of moving humans are greater than the number of humans required to form a crowd
                pass
            else:
                start_time = time.time()  # for timeout purposes, recording the time
                indices = random.sample(
                    range(len(self.dynamic_humans)), numHumans
                )  # randomly sample a few humans from the list of moving humans
                HALF_SIZE_X = self.MAP_X / 2.0 - self.MARGIN
                HALF_SIZE_Y = self.MAP_Y / 2.0 - self.MARGIN
                while (
                    True
                ):  # comes out of loop only when spawned object collides with none of current objects
                    if self.check_timeout(
                        start_time, period=0.5
                    ):  # times out if a crowd location cannot be found in 0.5 seconds. Such a small period is set so that it does not interrupt the normal flow of the environment unnecessarily trying to form a crowd
                        break
                    x = random.uniform(-HALF_SIZE_X, HALF_SIZE_X)
                    y = random.uniform(-HALF_SIZE_Y, HALF_SIZE_Y)
                    i = Human_Human_Interaction(
                        ...
                        self.HUMAN_DIAMETER,
                        self.MAX_ADVANCE_HUMAN,
                        self.INTERACTION_GOAL_RADIUS,
                        self.INTERACTION_NOISE_VARIANCE,
                    )
                    ...
                    if collides:
                        ...
                        for j in range(numHumans):
                            self.dynamic_humans[indices[j]].set_goal(
                                i.humans[j].x, i.humans[j].y
                            )  # setting goal
                            self.goals[self.dynamic_humans[indices[j]].id] = Plant(
                                None,
                                i.humans[j].x,
                                i.humans[j].y,
                                self.HUMAN_GOAL_RADIUS,
                            )  # recording the goals in the goal dict
                            self.dynamic_humans[indices[j]].policy = (
                                "orca"  # this works better, crowds get formed with a higher probability when using orca policy
                            )
                        self.upcoming_interaction = (
                            i  # storing the new human-human interaction object
                        )
                        self.humans_forming_crowd: Dict[int, Human] = (
                            {}
                        )  # dictionary mapping human.id -> human
                        self.id_to_index = {}
                        for ind, j in enumerate(indices):
                            self.humans_forming_crowd[self.dynamic_humans[j].id] = (
                                self.dynamic_humans[j]
                            )
    ...
    def check_almost_crowd_formed(self):
        ...
        for human in self.humans_forming_crowd.values():
            if (
                np.sqrt((i_x - human.x) ** 2 + (i_y - human.y) ** 2)
                > self.INTERACTION_RADIUS + self.HUMAN_DIAMETER / 2
            ):
    ...
    def finish_human_crowd_formation(self, make_approx_crowd=False):
        ...
        if not make_approx_crowd:
            ...
            for human in self.humans_forming_crowd.values():
                ...
                count += 1
                self.upcoming_interaction.humans[self.id_to_index[human.id]] = (
                    human  # replace the human in the interaction with current human
                )
                self.goals.pop(
                    human.id
                )  # remove the human's id from the goal dictionary
            ...
            if self.upcoming_interaction.type == "moving":
                ...
                self.static_interactions.append(
                    self.upcoming_interaction
                )  # add the new interaction to the static interaction list
            ...
            for ind, human in enumerate(self.humans_forming_crowd.values()):
                ...
                self.dynamic_humans.remove(human)  # remove human from list
                count += 1
                human.orientation = np.arctan2(
                    i_y - human.y, i_x - human.x
                )  # setting orientation to face the center of geometry
            assert len(self.dynamic_humans) + count == l  # sanity check
    ...
    def form_human_laptop_interaction(self):
        ...
        if (
            len(self.laptops) == 0 or len(self.dynamic_humans) == 0
        ):  # if no empty laptop, or no free moving human, do nothing
            pass
        else:
            index = random.sample(range(len(self.dynamic_humans)), 1)[
                0
            ]  # sampling a random human
            laptop_index = random.sample(range(len(self.laptops)), 1)[
                0
            ]  # sampling a random laptop
            self.h_l_forming_human = self.dynamic_humans[index]
            self.h_l_forming_human.policy = (
                "orca"  # using orca policy seems to work better
            )
            ...
            self.upcoming_h_l_interaction = Human_Laptop_Interaction(
                self.laptops[laptop_index],
                self.LAPTOP_WIDTH + self.HUMAN_LAPTOP_DISTANCE,
                self.HUMAN_DIAMETER,
            )
            ...
            if not collides:
                ...
                self.h_l_forming_human.set_goal(
                    self.upcoming_h_l_interaction.human.x,
                    self.upcoming_h_l_interaction.human.y,
                )
                self.goals[self.h_l_forming_human.id] = Plant(
                    None,
                    self.upcoming_h_l_interaction.human.x,
                    self.upcoming_h_l_interaction.human.y,
                    self.HUMAN_GOAL_RADIUS,
    ...
    def finish_h_l_formation(self):
        ...
        self.laptops.remove(
            self.upcoming_h_l_interaction.laptop
        )  # removing laptop from list
        ...
        self.h_l_interactions.append(
            self.upcoming_h_l_interaction
        )  # adding the new human laptop interaction
    ...
    def populate_prev_obs(self):
        ...
        for entity in (
            ...
            coordinates = self.get_robot_frame_coordinates(
                np.array([[entity.x, entity.y]], dtype=np.float32)
            ).flatten()
            ...
            self._prev_observations[entity.id] = EntityObs(
                entity.id, coordinates[0], coordinates[1], theta, sin_theta, cos_theta
            )
    ...
    def compute_reward_and_ticks(self, action):
        ...
        distance_to_goal = np.sqrt(
            (self.robot.goal_x - self.robot.x) ** 2
            + (self.robot.goal_y - self.robot.y) ** 2
        )
        ...
        if (not self.has_orca_robot_collided) and (
            ...
            distance_to_goal_orca_robot = np.sqrt(
                (self.robot_orca.goal_x - self.robot_orca.x) ** 2
                + (self.robot_orca.goal_y - self.robot_orca.y) ** 2
            )
            ...
            orca_robot_speed = np.linalg.norm(
                np.array(
                    [self.robot_orca.vel_x, self.robot_orca.vel_y], dtype=np.float32
                )
            )
        ...
        for i in self.static_interactions + self.moving_interactions:
            for h in i.humans:
                self.all_humans.append(h)

        for i in self.h_l_interactions:
            self.all_humans.append(i.human)
        ...
        for human in self.all_humans:
            ...
            vx = (
                ...
                - action[0] * np.cos(action[2] * self.TIMESTEP + self.robot.orientation)
                - action[1]
                * np.cos(action[2] * self.TIMESTEP + self.robot.orientation + np.pi / 2)
            )
            vy = (
                human.speed * np.sin(human.orientation)
                - action[0] * np.sin(action[2] * self.TIMESTEP + self.robot.orientation)
                - action[1]
                * np.sin(action[2] * self.TIMESTEP + self.robot.orientation + np.pi / 2)
            )
        ...
        for interaction in (
            ...
            vx = (
                speed * np.cos(human.orientation)
                - action[0] * np.cos(action[2] * self.TIMESTEP + self.robot.orientation)
                - action[1]
                * np.cos(action[2] * self.TIMESTEP + self.robot.orientation + np.pi / 2)
            )
            vy = (
                speed * np.sin(human.orientation)
                - action[0] * np.sin(action[2] * self.TIMESTEP + self.robot.orientation)
                - action[1]
                * np.sin(action[2] * self.TIMESTEP + self.robot.orientation + np.pi / 2)
            )
        ...
        if (
            self.MAP_X / 2 < self.robot.x
            or self.robot.x < -self.MAP_X / 2
        ...
        reward = self.reward_calculator.compute_reward(
            action, self._prev_observations, self._current_observations
        )
        ...
        robot_vx = action[0] * np.cos(self.robot.orientation) + action[1] * np.cos(
            self.robot.orientation + np.pi / 2
        )
        robot_vy = action[0] * np.sin(self.robot.orientation) + action[1] * np.sin(
            self.robot.orientation + np.pi / 2
        )

        for h in self.static_humans + self.dynamic_humans:
            closest_human_dist = min(
                closest_human_dist,
        ...
        for i in self.h_l_interactions:
            closest_human_dist = min(
                closest_human_dist,
                np.sqrt(
                    (self.robot.x - i.human.x) ** 2 + (self.robot.y - i.human.y) ** 2
                ),
            )
            h = i.human
            t = compute_time_to_collision(
        ...
        if (
            closest_human_dist - self.ROBOT_RADIUS
        ) >= 0.45:  # same value used in SEAN 2.0
        ...
        for p in self.plants:
            d = (
                np.sqrt((self.robot.x - p.x) ** 2 + (self.robot.y - p.y) ** 2)
                - self.PLANT_RADIUS
            )
        ...
        for table in self.tables:
            p_x, p_y = get_nearest_point_from_rectangle(
                table.x,
                table.y,
                table.length,
                table.width,
        ...
        for i, interaction in enumerate(
            self.moving_interactions + self.static_interactions
        ):
            interaction_indices = []
            count_of_humans = 0
            ...
            for p in range(len(interaction_indices)):
                for q in range(p + 1, len(interaction_indices)):
                    info["interactions"]["human-human"].append(
                        (interaction_indices[p], interaction_indices[q])
                    )
                    info["interactions"]["human-human"].append(
                        (interaction_indices[q], interaction_indices[p])
                    )

            curr_humans += count_of_humans

        for i, interaction in enumerate(self.h_l_interactions):
            info["interactions"]["human-laptop"].append(
                (curr_humans + i, curr_laptops + i)
            )
            # assertion statement
    ...
    def _get_walls_for_L_shaped_room(self, location: int) -> List[Wall]:
        ...
        if location == 0:
            w_l8 = Wall(
                id=None,
                x=self.MAP_X / 2 - self.L_X / 2,
                y=self.MAP_Y / 2 - self.L_Y,
                theta=np.pi,
                length=self.L_X,
            ...
            w_l7 = Wall(
                id=None,
                x=self.MAP_X / 2 - (self.WALL_THICKNESS / 2),
                y=-self.L_Y / 2,
                theta=np.pi / 2,
                length=self.MAP_Y - self.L_Y,
                thickness=self.WALL_THICKNESS,
            )
            w_l6 = Wall(
                id=None,
                x=self.MAP_X / 6,
                y=-self.MAP_Y / 2 + (self.WALL_THICKNESS / 2),
                theta=0,
                length=2 * self.MAP_X / 3,
                thickness=self.WALL_THICKNESS,
            )
            w_l5 = Wall(
                id=None,
            ...
            w_l2 = Wall(
                ...
                y=self.MAP_Y / 2 - (self.WALL_THICKNESS / 2),
                theta=np.pi,
                length=self.MAP_X - self.L_X,
                thickness=self.WALL_THICKNESS,
            )
            w_l1 = Wall(
                id=None,
                x=self.MAP_X / 2 - self.L_X,
                y=self.MAP_Y / 2 - self.L_Y / 2,
                theta=np.pi / 2,
                length=self.L_Y,
                thickness=self.WALL_THICKNESS,
            )
    ...
    def _add_corridors_to_L_shaped_room(self, location: int) -> None:
        ...
        if location == 0:
            ...
            gap1 = (
                random.random() * min_gap + min_gap
            )  # gap1 is sampled between min_gap and 2*min_gap
            gap2 = (
                random.random() * min_gap + min_gap
            )  # gap2 is sampled between min_gap and 2*min_gap
            gap1_center = random.random() * (self.MAP_X - gap1 - self.L_X) + (
                -self.MAP_X / 2 + gap1 / 2
            )  # center of gap1 is sampled between (-X/2 + gap1/2, X/2 - LX - gap1/2)
            w1 = Wall(
                None,
                ((-self.MAP_X / 2 + gap1_center - gap1 / 2) / 2),
                self.MAP_Y / 2 - self.L_Y,
                0,
                (gap1_center - gap1 / 2 + self.MAP_X / 2),
                self.WALL_THICKNESS,
            )
            w2 = Wall(
                None,
                (gap1_center + gap1 / 2 + self.MAP_X / 2 - self.L_X) / 2,
                self.MAP_Y / 2 - self.L_Y,
                0,
                (self.MAP_X / 2 - self.L_X - (gap1_center + gap1 / 2)),
                self.WALL_THICKNESS,
            )
            ...
            )  # adding this bit so that no obstacles are sampled in the gap
            gap2_center = random.random() * (self.MAP_Y - gap2 - self.L_Y) + (
                -self.MAP_Y / 2 + gap2 / 2
            )  # center of gap2 is sampled between (-Y/2 + gap2/2, Y/2 - LY - gap2/2)
            w3 = Wall(
                None,
                self.MAP_X / 2 - self.L_X,
                (-self.MAP_Y / 2 + gap2_center - gap2 / 2) / 2,
                np.pi / 2,
                (gap2_center - gap2 / 2 + self.MAP_Y / 2),
                self.WALL_THICKNESS,
            )
            w4 = Wall(
                None,
                self.MAP_X / 2 - self.L_X,
                (gap2_center + gap2 / 2 + self.MAP_Y / 2 - self.L_Y) / 2,
                np.pi / 2,
                (self.MAP_Y / 2 - self.L_Y - (gap2_center + gap2 / 2)),
                self.WALL_THICKNESS,
            )
            ...
            gap1_center = random.random() * (self.MAP_X - gap1 - self.L_X) + (
                -self.MAP_X / 2 + self.L_X + gap1 / 2
            )  # center of gap1 is sampled between (-X/2 + LX + gap1/2, X/2 - gap1/2)
            w1 = Wall(
                None,
                ((-self.MAP_X / 2 + self.L_X + gap1_center - gap1 / 2) / 2),
                self.MAP_Y / 2 - self.L_Y,
                0,
                (gap1_center - gap1 / 2 + self.MAP_X / 2 - self.L_X),
                self.WALL_THICKNESS,
            )
            w2 = Wall(
                None,
                (gap1_center + gap1 / 2 + self.MAP_X / 2) / 2,
                self.MAP_Y / 2 - self.L_Y,
                0,
                (self.MAP_X / 2 - (gap1_center + gap1 / 2)),
                self.WALL_THICKNESS,
            )
            self.walls.append(w1)
            self.objects.append(w1)
            ...
            gap2_center = random.random() * (self.MAP_Y - gap2 - self.L_Y) + (
                -self.MAP_Y / 2 + gap2 / 2 + self.L_Y
            )  # center of gap2 is sampled between (-Y/2 + gap2/2 + LY, Y/2 - gap2/2)
            w3 = Wall(
                None,
                self.MAP_X / 2 - self.L_X,
                (-self.MAP_Y / 2 + self.L_Y + gap2_center - gap2 / 2) / 2,
                np.pi / 2,
                (gap2_center - gap2 / 2 + self.MAP_Y / 2 - self.L_Y),
                self.WALL_THICKNESS,
            )
            w4 = Wall(
                None,
                self.MAP_X / 2 - self.L_X,
                (gap2_center + gap2 / 2 + self.MAP_Y / 2) / 2,
                np.pi / 2,
                (self.MAP_Y / 2 - (gap2_center + gap2 / 2)),
                self.WALL_THICKNESS,
            )
            self.walls.append(w3)
            self.objects.append(w3)
    ...
    def _add_corridors_to_rectangular_shaped_room(self):
        ...
        gap1_center = random.random() * (
            self.MAP_X / 2 - gap1 / 2
        )  # center of gap1 is sampled between (-X/2 + gap1/2, X/2 - gap1/2)
        w1 = Wall(
            None,
            ((-self.MAP_X / 2 + gap1_center - gap1 / 2) / 2),
            -self.MAP_Y / 2 + self.MAP_Y / 3,
            0,
            (gap1_center - gap1 / 2 + self.MAP_X / 2),
            self.WALL_THICKNESS,
        )
        ...
        gap2_center = random.random() * (
            self.MAP_X / 2 - gap2 / 2
        )  # center of gap2 is sampled between (-X/2 + gap2/2, X/2 - gap2/2)
        w3 = Wall(
            None,
            ((-self.MAP_X / 2 + gap2_center - gap2 / 2) / 2),
            -self.MAP_Y / 2 + 2 * self.MAP_Y / 3,
            0,
            (gap2_center - gap2 / 2 + self.MAP_X / 2),
            self.WALL_THICKNESS,
        )
        w4 = Wall(
            None,
            (gap2_center + gap2 / 2 + self.MAP_X / 2) / 2,
            -self.MAP_Y / 2 + 2 * self.MAP_Y / 3,
            0,
            (self.MAP_X / 2 - (gap2_center + gap2 / 2)),
            self.WALL_THICKNESS,
        )
        self.walls.append(w3)
        self.objects.append(w3)
    ...
    def _add_walls(self):
        if self.shape == "L":
            ...
            self.L_X = (
                (2 if (self.location == 0 or self.location == 3) else 1)
                * self.MAP_X
                / 3
            )
            self.L_Y = (
                (1 if (self.location == 0 or self.location == 3) else 2)
                * self.MAP_Y
                / 3
            )
            ...
            l = Laptop(
                id=None,
                x=(-1 if (self.location == 1 or self.location == 3) else 1)
                * self.MAP_X
                / 2.0
                + (-1 if (self.location == 0 or self.location == 2) else 1)
                * self.L_X
                / 2.0,
                y=(-1 if (self.location == 2 or self.location == 3) else 1)
                * self.MAP_Y
                / 2.0
                + (-1 if (self.location == 0 or self.location == 1) else 1)
                * self.L_Y
                / 2.0,
                width=self.L_Y,
                length=self.L_X,
        ...
        # walls (hardcoded to be at the boundaries of the environment)
            ...
            w1 = Wall(
                None,
                self.MAP_X / 2 - self.WALL_THICKNESS / 2,
                0,
                -np.pi / 2,
                self.MAP_Y,
                self.WALL_THICKNESS,
            ...
            w2 = Wall(
                None,
                0,
                -self.MAP_Y / 2 + self.WALL_THICKNESS / 2,
                -np.pi,
                self.MAP_X,
                self.WALL_THICKNESS,
            )
        ...
        if (
            self.add_corridors
        ):  # corridors are hard coded to be at Y/3 and 2Y/3 where Y is the room's length along Y direction
    ...
    def _get_kwargs(self, object_type: SocNavGymObject, extra_info: dict = None):
        ...
        if object_type == SocNavGymObject.ROBOT:
            ...
            object_type == SocNavGymObject.STATIC_HUMAN
            or object_type == SocNavGymObject.DYNAMIC_HUMAN
        ):
            policy = self.HUMAN_POLICY
            if policy == "random":
                policy = random.choice(["sfm", "orca"])
            human_speed = 0
            human_type = "static"
            ...
            if edge == 0:
                center = (
                    table.x
                    + np.cos(table.orientation + np.pi / 2)
                    * (self.TABLE_WIDTH - self.LAPTOP_WIDTH)
                    / 2,
                    table.y
                    + np.sin(table.orientation + np.pi / 2)
                    * (self.TABLE_WIDTH - self.LAPTOP_WIDTH)
                    / 2,
                )
                ...
                center = (
                    table.x
                    + np.cos(table.orientation + np.pi)
                    * (self.TABLE_LENGTH - self.LAPTOP_LENGTH)
                    / 2,
                    table.y
                    + np.sin(table.orientation + np.pi)
                    * (self.TABLE_LENGTH - self.LAPTOP_LENGTH)
                    / 2,
                )
                ...
                center = (
                    table.x
                    + np.cos(table.orientation)
                    * (self.TABLE_LENGTH - self.LAPTOP_LENGTH)
                    / 2,
                    table.y
                    + np.sin(table.orientation)
                    * (self.TABLE_LENGTH - self.LAPTOP_LENGTH)
                    / 2,
                )
                theta = table.orientation + np.pi / 2
            ...
            object_type == SocNavGymObject.HUMAN_HUMAN_INTERACTION_DYNAMIC
            or object_type
            == SocNavGymObject.HUMAN_HUMAN_INTERACTION_DYNAMIC_NON_DISPERSING
            or object_type == SocNavGymObject.HUMAN_HUMAN_INTERACTION_STATIC
            or object_type
            == SocNavGymObject.HUMAN_HUMAN_INTERACTION_STATIC_NON_DISPERSING
        ):
            ...
            object_type == SocNavGymObject.HUMAN_LAPTOP_INTERACTION
    ...
    def _sample_object(
        self, start_time: float, object_type: SocNavGymObject, extra_info=None
    ):
        object_class = object_type.get_class()
        while True:
            if self.check_timeout(start_time):
                print(
                    f"timed out spawning object of type {object_type.name}, starting again"
                )
                return None
            ...
            if object_type == SocNavGymObject.LAPTOP:
                ...
                objects_to_check_collision = self.objects
            for (
                obj
            ) in (
                objects_to_check_collision
            ):  # check if spawned object collides with any of the exisiting objects
    ...
    def _sample_human_laptop_interaction(
        self, start_time: float, object_type: SocNavGymObject
    ):
        while True:
            if self.check_timeout(start_time):
                print(
    ...
    def try_reset(self, seed=None, options=None):
        ...
        if not self.has_configured:
            raise Exception(
                'Please pass in the keyword argument config="path to config" while calling gym.make'
            )
        ...
        self.plants = []
        self.tables = []
        self.goals: Dict[int, Plant] = (
            {}
        )  # dictionary to store all the goals. The key would be the id of the entity. The goal would be a Plant object so that collision checks can be done.
        ...
        # dynamic humans
        for _ in range(
            self.NUMBER_OF_DYNAMIC_HUMANS
        ):  # spawn specified number of humans
        ...
        if len(self.tables) == 0:
            ...
            self.NUMBER_OF_LAPTOPS
            + self.NUMBER_OF_H_L_INTERACTIONS
            + self.NUMBER_OF_H_L_INTERACTIONS_NON_DISPERSING
            > 4 * self.NUMBER_OF_TABLES
        ):
            raise AssertionError(
                "Number of laptops exceeds the number of edges available on tables"
            )
        else:
        ...
        for ind in range(self.NUMBER_OF_H_H_DYNAMIC_INTERACTIONS):
            i = self._sample_object(
                start_time,
                SocNavGymObject.HUMAN_HUMAN_INTERACTION_DYNAMIC,
        ...
        for ind in range(self.NUMBER_OF_H_H_STATIC_INTERACTIONS):
            i = self._sample_object(
                start_time,
                SocNavGymObject.HUMAN_HUMAN_INTERACTION_STATIC,
                extra_info={"index": ind},
            )
            if i == None:
                return False, None, None
        ...
        for _ in range(self.NUMBER_OF_H_L_INTERACTIONS):
            # sampling a laptop
            laptop, interaction = self._sample_human_laptop_interaction(
                start_time, SocNavGymObject.HUMAN_LAPTOP_INTERACTION
            )
        ...
        for _ in range(self.NUMBER_OF_H_L_INTERACTIONS_NON_DISPERSING):
            # sampling a laptop
            laptop, interaction = self._sample_human_laptop_interaction(
                start_time, SocNavGymObject.HUMAN_LAPTOP_INTERACTION_NON_DISPERSING
            )
            if laptop == None or interaction == None:
                return False, None, None
            self.h_l_interactions.append(interaction)
        ...
        for human in self.static_humans:
            ...
            human.set_goal(
                human.x, human.y
            )  # setting goal of static humans to where they are spawned
        ...
        self.prev_goal_distance = np.sqrt(
            (self.robot.x - self.robot.goal_x) ** 2
            + (self.robot.y - self.robot.goal_y) ** 2
        )
    ...
    def render_without_showing(self, mode="human", draw_human_gaze=False):
        ...
        self.world_image = (
            np.ones((int(self.RESOLUTION_Y), int(self.RESOLUTION_X), 3)) * 255
        ).astype(np.uint8)

        # can be used for debugging.
        if draw_human_gaze:
            for human in self.static_humans + self.dynamic_humans:
                human.draw_gaze_range(
                    self.world_image,
                    self.HUMAN_GAZE_ANGLE,
                    self.PIXEL_TO_WORLD_X,
                    self.PIXEL_TO_WORLD_Y,
                    self.MAP_X,
                    self.MAP_Y,
                )
        ...
        cv2.circle(
            ...
            int(
                w2px(self.robot.x + self.GOAL_RADIUS, self.PIXEL_TO_WORLD_X, self.MAP_X)
                - w2px(self.robot.x, self.PIXEL_TO_WORLD_X, self.MAP_X)
            ),
            (0, 255, 0),
            2,
        )
        ...
        for human in self.dynamic_humans:  # only draw goals for the dynamic humans
            cv2.circle(
                self.world_image,
                (
                    w2px(human.goal_x, self.PIXEL_TO_WORLD_X, self.MAP_X),
                    w2py(human.goal_y, self.PIXEL_TO_WORLD_Y, self.MAP_Y),
                ),
                int(
                    w2px(
                        human.x + self.HUMAN_GOAL_RADIUS,
                        self.PIXEL_TO_WORLD_X,
                        self.MAP_X,
                    )
                    - w2px(human.x, self.PIXEL_TO_WORLD_X, self.MAP_X)
                ),
                (120, 0, 0),
                2,
            )
        ...
        for i in self.moving_interactions:
            cv2.circle(
                self.world_image,
                (
                    w2px(i.goal_x, self.PIXEL_TO_WORLD_X, self.MAP_X),
                    w2py(i.goal_y, self.PIXEL_TO_WORLD_Y, self.MAP_Y),
                ),
                int(
                    w2px(i.x + i.goal_radius, self.PIXEL_TO_WORLD_X, self.MAP_X)
                    - w2px(i.x, self.PIXEL_TO_WORLD_X, self.MAP_X)
                ),
                (0, 0, 255),
                2,
            )
    ...
    def record(self, path: str):
        ...
        self.robot.draw(
            img, self.PIXEL_TO_WORLD_X, self.PIXEL_TO_WORLD_Y, self.MAP_X, self.MAP_Y
        )
        ...
        if self._is_terminated or self._is_truncated:
            output = cv2.VideoWriter(
                path, cv2.VideoWriter_fourcc("m", "p", "4", "v"), 20, (width, height)
            )
