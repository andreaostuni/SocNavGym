...
class NoisyObservations(gym.Wrapper):
    def __init__(
        ...
        mean,
        std_dev,
        apply_noise_to=["robot", "humans", "tables", "laptops", "plants", "walls"],
    ) -> None:
        """
        ...
        for entity in self.apply_noise_to:
            assert (
                ...
                or entity == "tables"
                or entity == "walls"
            ), 'apply_noise_to only have the following names: "goal" "humans" "tables" "laptops" "plants" "walls"'
            ...
            "robot": spaces.Box(
                low=np.array(
                    [
                        ...
                        0,
                        0,
                        -self.MAP_X * np.sqrt(2) - self.max_noise,
                        -self.MAP_Y * np.sqrt(2) - self.max_noise,
                        -self.ROBOT_RADIUS - self.max_noise,
                    ],
                    dtype=np.float32,
                ),
                high=np.array(
            ...
            d["humans"] = spaces.Box(
                low=np.array(
                    [
                        ...
                        -1.0 - self.max_noise,
                        -1.0 - self.max_noise,
                        -self.HUMAN_DIAMETER / 2 - self.max_noise,
                        -(self.MAX_ADVANCE_HUMAN + self.MAX_ADVANCE_ROBOT) * np.sqrt(2)
                        - self.max_noise,
                        -2 * np.pi / self.TIMESTEP - self.max_noise,
                        0 - self.max_noise,
                    ]
                    * (
                        (
            ...
            d["laptops"] = spaces.Box(
                low=np.array(
                    [
                        ...
                        -1.0 - self.max_noise,
                        -1.0 - self.max_noise,
                        -self.LAPTOP_RADIUS - self.max_noise,
                        -(self.MAX_ADVANCE_ROBOT) * np.sqrt(2) - self.max_noise,
                        -self.MAX_ROTATION - self.max_noise,
                        0 - self.max_noise,
                    ]
                    * (
                        (
            ...
            d["tables"] = spaces.Box(
                low=np.array(
                    [
                        ...
                        -1.0 - self.max_noise,
                        -1.0 - self.max_noise,
                        -self.TABLE_RADIUS - self.max_noise,
                        -(self.MAX_ADVANCE_ROBOT) * np.sqrt(2) - self.max_noise,
                        -self.MAX_ROTATION - self.max_noise,
                        0 - self.max_noise,
                    ]
            ...
            d["plants"] = spaces.Box(
                low=np.array(
                    [
                        ...
                        -1.0 - self.max_noise,
                        -1.0 - self.max_noise,
                        -self.PLANT_RADIUS - self.max_noise,
                        -(self.MAX_ADVANCE_ROBOT) * np.sqrt(2) - self.max_noise,
                        -self.MAX_ROTATION - self.max_noise,
                        0 - self.max_noise,
                    ]
                ...
                d["walls"] = spaces.Box(
                    low=np.array(
                        [
                            ...
                            -1.0 - self.max_noise,
                            -1.0 - self.max_noise,
                            -self.WALL_SEGMENT_SIZE - self.max_noise,
                            -(self.MAX_ADVANCE_ROBOT) * np.sqrt(2) - self.max_noise,
                            -self.MAX_ROTATION - self.max_noise,
                            0 - self.max_noise,
                        ]
    ...
    def add_noise(self, obs):
        ...
        if not self.env.get_padded_observations:
            ...
            for entity_name in self.apply_noise_to:
                if entity_name == "robot" or entity_name == "walls":
                    continue
                entity_list.append(entity_name)
        for entity in entity_list:
            # if the entity is not present in the observation, then continue
            if entity not in noisy_obs.keys():
                continue

            o = noisy_obs[entity].reshape(-1, self.env.entity_obs_dim)
            for i in range(o.shape[0]):
                for j in range(
                    encoding_size, self.env.entity_obs_dim
                ):  # noise is only added to the non-one-hot components of the observation
