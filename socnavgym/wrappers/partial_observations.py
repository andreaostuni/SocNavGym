...
class PartialObservations(gym.Wrapper):
    def __init__(self, env: SocNavEnv_v1, fov_angle: float, range: float) -> None:
        ...
        self.range = range
        assert (
            self.fov_angle <= 2 * np.pi and self.fov_angle >= 0
        ), "FOV angle should be between 0 and 2*pi"
        self.num_humans = 0
        self.num_plants = 0
        self.num_tables = 0
        self.num_laptops = 0
    ...
    def observation_space(self):
        ...
        for name in ["humans", "plants", "tables", "laptops", "walls"]:
            self.is_entity_present[name] = True
        for e_name in ["humans", "plants", "tables", "laptops", "walls"]:
            ...
            for i in range(e_obs.shape[0]):
                ...
                if (
                    ...
                    continue
                if self.lies_in_range(e_obs[i]):
                    partial_obs = np.concatenate((partial_obs, e_obs[i])).flatten()
        ...
        if not self.env.get_padded_observations and self.num_humans == 0:
            self.is_entity_present["humans"] = False
        if not self.env.get_padded_observations and self.num_tables == 0:
            self.is_entity_present["tables"] = False
        if not self.env.get_padded_observations and self.num_laptops == 0:
            self.is_entity_present["laptops"] = False
        if not self.env.get_padded_observations and self.num_plants == 0:
            self.is_entity_present["plants"] = False
        if not self.env.get_padded_observations and self.num_walls > 0:
            self.is_entity_present["walls"] = True
        else:
            self.is_entity_present["walls"] = False
        ...
        if not self.get_padded_observations:
            ...
            if self.is_entity_present["walls"]:
                d["walls"] = spaces.Box(
                    ...
                    shape=(
                        ((self.robot.one_hot_encoding.shape[0] + 8) * self.num_walls,)
                    ),
                    dtype=np.float32,
                )
    ...
    def lies_in_range(self, obs):
        ...
        if (
            (np.arctan2(obs[7], obs[6]) >= -self.fov_angle / 2)
            and (np.arctan2(obs[7], obs[6]) <= self.fov_angle / 2)
            and (np.linalg.norm([obs[6], obs[7]]) <= self.range)
        ):
            return True
        else:
            return False
    ...
    def lies_in_range_using_coordinates(self, x, y):
        ...
        if (
            (np.arctan2(coord[1], coord[0]) >= -self.fov_angle / 2)
            and (np.arctan2(coord[1], coord[0]) <= self.fov_angle / 2)
            and (np.linalg.norm([coord[0], coord[1]]) <= self.range)
        ):
            return True
        else:
            return False
    ...
    def get_partial_observation(self, obs):
        ...
        for entity_name in ["humans", "plants", "tables", "laptops"]:
            if entity_name not in obs.keys():
                continue
            ...
            for i in range(o.shape[0]):
                ...
                if self.lies_in_range(o[i]):
                    partial_obs = np.concatenate((partial_obs, o[i])).flatten()
        ...
        if self.num_humans == 0 and "humans" in d.keys():
            d.pop("humans")
        if self.num_plants == 0 and "plants" in d.keys():
            d.pop("plants")
        if self.num_laptops == 0 and "laptops" in d.keys():
            d.pop("laptops")
        if self.num_tables == 0 and "tables" in d.keys():
            d.pop("tables")
        if self.num_walls == 0 and "walls" in d.keys():
            d.pop("walls")
        return d
    ...
    def get_interaction_info(self):
        ...
        for h in self.env.static_humans + self.env.dynamic_humans:
            if self.lies_in_range_using_coordinates(h.x, h.y):
                curr_humans += 1

        for l in self.env.laptops:
            if self.lies_in_range_using_coordinates(l.x, l.y):
                curr_laptops += 1

        for i, interaction in enumerate(
            self.env.moving_interactions + self.env.static_interactions
        ):
            interaction_indices = []
            count_of_humans = 0
        ...
        for i, interaction in enumerate(self.env.h_l_interactions):
            if self.lies_in_range_using_coordinates(
                interaction.human.x, interaction.human.y
            ) and self.lies_in_range_using_coordinates(
                interaction.laptop.x, interaction.laptop.y
            ):
                human_laptop.append(
                    (curr_humans + curr_count, curr_laptops + curr_laptop_count)
                )
    ...
    def pad_observations(self, obs):
        ...
        for entity_name in ["humans", "plants", "tables", "laptops"]:
            if entity_name in obs:
                obs[entity_name] = np.concatenate(
                    (
                        obs[entity_name],
                        np.zeros(
                            self.observation_space[entity_name].shape[0]
                            - obs[entity_name].shape[0]
                        ),
                    ),
                    dtype=np.float32,
                )
    ...
    def render(self, mode="human"):
        if not self.env.window_initialised:
            ...
            cv2.resizeWindow(
                "world", int(self.env.RESOLUTION_VIEW), int(self.env.RESOLUTION_VIEW)
            )
            self.env.window_initialised = True
            self.env.world_image = (
                np.ones((int(self.env.RESOLUTION_Y), int(self.env.RESOLUTION_X), 3))
                * 255
            ).astype(np.uint8)

        self.env.world_image = (
            np.ones((int(self.env.RESOLUTION_Y), int(self.env.RESOLUTION_X), 3)) * 255
        ).astype(np.uint8)
        ...
        cv2.circle(
            self.env.world_image,
            (
                w2px(self.env.robot.goal_x, self.env.PIXEL_TO_WORLD_X, self.env.MAP_X),
                w2py(self.env.robot.goal_y, self.env.PIXEL_TO_WORLD_Y, self.env.MAP_Y),
            ),
            int(
                w2px(
                    self.env.robot.x + self.env.GOAL_RADIUS,
                    self.env.PIXEL_TO_WORLD_X,
                    self.env.MAP_X,
                )
                - w2px(self.env.robot.x, self.env.PIXEL_TO_WORLD_X, self.env.MAP_X)
            ),
            (0, 255, 0),
            2,
        )
        ...
        for human in self.env.dynamic_humans:  # only draw goals for the dynamic humans
            cv2.circle(
                self.env.world_image,
                (
                    w2px(human.goal_x, self.env.PIXEL_TO_WORLD_X, self.env.MAP_X),
                    w2py(human.goal_y, self.env.PIXEL_TO_WORLD_Y, self.env.MAP_Y),
                ),
                int(
                    w2px(
                        human.x + self.env.HUMAN_GOAL_RADIUS,
                        self.env.PIXEL_TO_WORLD_X,
                        self.env.MAP_X,
                    )
                    - w2px(human.x, self.env.PIXEL_TO_WORLD_X, self.env.MAP_X)
                ),
                (120, 0, 0),
                2,
            )
        ...
        for i in self.env.moving_interactions:
            cv2.circle(
                self.env.world_image,
                (
                    w2px(i.goal_x, self.env.PIXEL_TO_WORLD_X, self.env.MAP_X),
                    w2py(i.goal_y, self.env.PIXEL_TO_WORLD_Y, self.env.MAP_Y),
                ),
                int(
                    w2px(i.x + i.goal_radius, self.env.PIXEL_TO_WORLD_X, self.env.MAP_X)
                    - w2px(i.x, self.env.PIXEL_TO_WORLD_X, self.env.MAP_X)
                ),
                (0, 0, 255),
                2,
            )
        ...
        for i in (
            self.env.moving_interactions
            + self.env.static_interactions
            + self.env.h_l_interactions
        ):
            i.draw(
                self.env.world_image,
                self.env.PIXEL_TO_WORLD_X,
