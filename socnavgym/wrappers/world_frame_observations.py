...
class WorldFrameObservations(gym.Wrapper):
    ...
    def observation_space(self):
        ...
        if self.env.is_entity_present["humans"]:
            d["humans"] = spaces.Box(
                low=np.array(
                    ...
                    * (
                        (
                            self.env.MAX_HUMANS
                            + (
                                self.env.MAX_H_L_INTERACTIONS
                                + self.env.MAX_H_L_INTERACTIONS_NON_DISPERSING
                            )
                            + (
                                (
                                    self.env.MAX_H_H_DYNAMIC_INTERACTIONS
                                    + self.env.MAX_H_H_DYNAMIC_INTERACTIONS_NON_DISPERSING
                                )
                                * self.env.MAX_HUMAN_IN_H_H_INTERACTIONS
                            )
                            + (
                                (
                                    self.env.MAX_H_H_STATIC_INTERACTIONS
                                    + self.env.MAX_H_H_STATIC_INTERACTIONS_NON_DISPERSING
                                )
                                * self.env.MAX_HUMAN_IN_H_H_INTERACTIONS
                            )
                        )
                        if self.env.get_padded_observations
                        else self.env.total_humans
                    ),
                    dtype=np.float32,
                ),
                high=np.array(
                ...
                shape=(
                    (
                        (self.env.robot.one_hot_encoding.shape[0] + 8)
                        * (
                            (
                                self.env.MAX_HUMANS
                                + (
                ...
                low=np.array(
                    ...
                    * (
                        ...
                        if self.env.get_padded_observations
                            ...
                            self.env.NUMBER_OF_LAPTOPS
                            + (
                                self.env.NUMBER_OF_H_L_INTERACTIONS
                                + self.env.NUMBER_OF_H_L_INTERACTIONS_NON_DISPERSING
                            )
                        )
                    ),
                    dtype=np.float32,
            ...
            d["tables"] = spaces.Box(
                low=np.array(
                    ...
                    * (
                        self.env.MAX_TABLES
                        if self.env.get_padded_observations
                        else self.env.NUMBER_OF_TABLES
                    ),
                    dtype=np.float32,
                ),
                high=np.array(
            ...
            d["plants"] = spaces.Box(
                low=np.array(
                    ...
                    * (
                        self.env.MAX_PLANTS
                        if self.env.get_padded_observations
                        else self.env.NUMBER_OF_PLANTS
                    ),
                    dtype=np.float32,
                ),
                high=np.array(
        ...
        if not self.env.get_padded_observations:
            ...
            for w in self.env.walls:
                ...
                if w.length % self.env.WALL_SEGMENT_SIZE != 0:
                    total_segments += 1
    ...
    def _get_entity_obs(self, object):
        ...
        def _get_wall_obs(wall: Wall, size: float):
            ...
            for center, length in zip(centers, lengths):
                ...
                obs = np.concatenate(
                    (obs, np.array([[center[0], center[1]]]).flatten())
                )
                # sin and cos of angles
                obs = np.concatenate(
                    (
                        obs,
                        np.array([np.sin(wall.orientation), np.cos(wall.orientation)]),
                    )
                )
        ...
        output = np.concatenate(
            (output, np.array([[object.x, object.y]]).flatten()), dtype=np.float32
        )
        ...
        output = np.concatenate((output, speeds), dtype=np.float32)
    ...
    def _get_world_frame_obs(self):
        ...
        robot_obs = np.concatenate(
            (self.env.robot.one_hot_encoding, robot_obs), dtype=np.float32
        )
        # adding radius to robot observation
        robot_obs = np.concatenate(
            (robot_obs, np.array([self.env.ROBOT_RADIUS])), dtype=np.float32
        ).flatten()
