...
ap.add_argument(
    "-j", "--joystick_id", required=False, default=0, help="Joystick identifier"
)
ap.add_argument(
    "-c",
    "--config",
    required=False,
    default="./environment_configs/exp1_no_sngnn.yaml",
...
ap.add_argument(
    ...
    required=False,
    default=False,
    help="Whether you want to record the observations, and actions or not",
)
ap.add_argument(
    "-s", "--start", required=False, default=0, help="starting episode number"
)
...
try:
    ...
    T = 5.0
    print(
        f"Move the joystick around for {T} seconds trying to reach the max and min values for the axes"
    )
    t = time.time()
    while time.time() - t < T:
        pygame.event.pump()
...
for episode in range(episodes):
    ...
    while not done:
        ...
        print(
            f"sngnn_reward: {sngnn_reward} ",
            f"distance_reward: {distance_reward} " f"total reward: {rew}",
        )
