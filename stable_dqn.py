...
class CometMLCallback(CheckpointCallback):
    ...
    def __init__(
        self, run_name: str, save_path: str, project_name: str, api_key: str, verbose=0
    ):
        # super(CometMLCallback, self).__init__(verbose)
        super(CometMLCallback, self).__init__(
            save_freq=25000, save_path=save_path, verbose=verbose
        )
        print("Logging using comet_ml")
        self.run_name = run_name
        self.experiment = Experiment(
            api_key=api_key, project_name=project_name, parse_args=False
        )
    ...
    def _on_rollout_end(self) -> None:
        ...
        metrics = {
            "rollout/ep_rew_mean": safe_mean(
                [ep_info["r"] for ep_info in self.locals["self"].ep_info_buffer]
            ),
            "rollout/ep_len_mean": safe_mean(
                [ep_info["l"] for ep_info in self.locals["self"].ep_info_buffer]
            ),
        }
        if len(self.locals["self"].ep_success_buffer) > 0:
            metrics["rollout/success_rate"] = safe_mean(
                self.locals["self"].ep_success_buffer
            )
...
ap.add_argument(
    "-a", "--api_key", help="api key to your comet ml profile", required=True
)
ap.add_argument(
    "-d",
    "--use_deep_net",
    help="True or False, based on whether you want a transformer based feature extractor",
    required=False,
    default=False,
)
...
model = DQN(
    "MultiInputPolicy", env, verbose=0, policy_kwargs=policy_kwargs, device=device
)
callback = CometMLCallback(
    args["run_name"], args["save_path"], args["project_name"], args["api_key"]
)
